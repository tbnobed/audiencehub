...
The dashboard date default is resolved at read time when unset: `APP_ENV=development` shows the seeded 2020-01-01 through 2024-12-31 range; Docker `APP_ENV=production` defaults to a rolling 90 days even with seeded data. To show the seeded range in Docker, an authenticated admin can PATCH `/api/admin/settings` with JSON `{"dashboard_default_preset":"custom","dashboard_default_from":"2020-01-01","dashboard_default_to":"2024-12-31"}` (using the usual session and CSRF token). The setting persists until changed; PATCH `{"dashboard_default_preset":"90d"}` to restore rolling 90 days. A fiscal-month-only PATCH does not change the range.
